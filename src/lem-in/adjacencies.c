...
static char			**matrix_adj(char **t_link *link, int count_vertex)
{
	while (link)
	{
...
static void			list_adj(t_lem_in *li)
{
	...
	while (i < li->count_vertex)
	{
		...
		adj = (t_list_adj *)ft_memalloc(sizeof(t_list_adj) * count_links);
		if (!adj)
			error(strerror(errno));
...
void 				adjacencies(t_lem_in *li)
{
	...
	if (!matrix_adj)
		error(strerror(errno)); = matrix_adj(li->first_link, li->count_vertex);
