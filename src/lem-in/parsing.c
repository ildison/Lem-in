...
/*   Updated: 2019/11/12 16:36:26 by cormund          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		add_new_link(t_link **first_link, t_vertex *vertex, char *line)
{
	...
	link->vertex_a = get_vertex(vertex, split_line[1]);
	ft_free_2x_mas((void **)split_line);
	if (*first_link)
...
void			parsing(t_lem_in *li)
{
	...
	if (input)
		while (input->line)
		{
			if (input->type == LI_START || input->type == LI_END || input->type == LI_VERTEX)
			{
				vertex = new_vertex(input->line, LI_START);
				add_vertex(&li->start, vertex, LI_START);
			}
