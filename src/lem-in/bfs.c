...
/*   Updated: 2019/12/03 14:25:26 by cormund          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_vertex	*marked_adjacent(t_queue **queue, t_lem_in *li, t_vertex *vrx, t_queue **last)
{
	int			i;

	i = LI_COUNTER;
	while (++i < LI_COUNT_ADJACENTS)
		if (get_edge(li, vrx, vrx->adj[i]) == 1 && !vrx->adj[i]->marked)
		{
